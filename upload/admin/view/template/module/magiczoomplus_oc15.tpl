  ...
  <?php trial_bage(); ?>
    ...
    <?php
        ...
        foreach ($parameters as $name => $s) {
                        ...
                        if ($s['id'] == 'disable-expand' || $s['id'] == 'disable-zoom') continue;
                            ...
                            switch($s["type"]) {
                                case "array":
                                        ...
                                        foreach($s["values"] as $p) {
                                            ...
                                            $rButtons[strtolower($p)] = '<label><input type="radio" value="'.$p.'"'. ($s["value"]==$p?"checked=\"checked\"":"").' name="'.$s['id'].'" id="magiczoomplussettings'. ucwords(strtolower($name)).$p.'">';
                                            $pName = ucwords($p);
                                            if(strtolower($p) == "yes")
                                ...
                                default:
                                    ...
                                    if (!isset($s["value"])) $s["value"] = $s["default"];
                                    $html .= '<input '.$width.' type="text" name="'.$s['id'].'" id="magiczoomplussettings'. ucwords(strtolower($name)).'" value="'.$s["value"].'" />';
                                    break;
                            }
